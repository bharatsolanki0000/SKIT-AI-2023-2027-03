...
Parses the Git commit history of the current repository and produces a
"Weekly Progress Report (Form-3)" PDF summarizing, per team member:
  - number of commits (and % share of total)
  - lines added / deleted / net LOC
  - active days
  - a timeline chart and a net-LOC bar chart
  - a detailed per-student commit log table with a blank mentor-marks column

Usage:
    python generate_report.py weekly    # commits from the last 7 days (default)
    python generate_report.py monthly   # commits from the last 30 days
    python generate_report.py final     # entire commit history
"""
...
def get_git_metrics(interval="weekly"):
    """
    Parses `git log` output for the requested interval.
...
def create_charts(students, timeline_activity, interval):
    """Generates the timeline and net-LOC charts as a single Image flowable."""
...
def generate_pdf(interval="weekly"):
    ...
    msg_style = ParagraphStyle('MsgStyle', parent=styles['Normal'],
        fontSize=8, leading=10, textColor=colors.HexColor("#1E293B"))
    meta_cell_style = ParagraphStyle('MetaCellStyle', parent=styles['Normal'],
        fontSize=8, leading=10, textColor=colors.HexColor("#475569"), alignment=1)
    marks_style = ParagraphStyle('MarksStyle', parent=styles['Normal'],
        fontSize=9, leading=12, textColor=colors.HexColor("#0F172A"), alignment=1)
    sig_block_style = ParagraphStyle('SigBlockStyle', parent=styles['Normal'],
